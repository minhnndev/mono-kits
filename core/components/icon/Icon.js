...
const useSource = name => {
  ...
  useEffect(() => {
    ...
    if (
      isRendered.current &&
      !isEqual(source, getIconExistedInApp(name)) &&
      !isUnMount.current
    ) {
      setSource(getIconExistedInApp(name));
    }
...
const getWidthHeight = style => {
  const mergeStyle = Array.isArray(style)
    ? style.reduce((obj, item) => (obj = {...obj, ...item}), {})
    : style;
  return {
    width: get(mergeStyle, 'width', DynamicSize(30)),
    height: get(mergeStyle, 'height', DynamicSize(30)),
    marginBottom: 0,
  };
};
...
const Icon = ({name, style, ...props}) => {
  ...
  return source === loading ? (
    <View style={[{backgroundColor: '#efefef'}, getWidthHeight(style)]} />
  ) : (
    <Image
      {...props}
      {...tintColor}
