...
const Image = memo(
  React.forwardRef(
    (
      ...
      const {url, isLocalImage} = useMemo(
        () => getUrl(source, timeCache),
        [source],
      );
      ...
      return loading || isShowLoading ? (
        ...
        <AnimatedImage
          ...
          style={StyleSheet.flatten([
            isLocalImage && getDefaultSize(url),
            restProps.style,
          ])}
          fallback={!cached}
          ...
          accessibilityLabel={
            ...
            `Image|${getUrl(source, null)?.url?.uri}`
          }>
...
const ProgressSiveImage = ({
  ...
  const {url, isLocalImage} = useMemo(
    () => getUrl(isScale ? scale : original, timeCache),
    [source],
  );
  ...
  const thumbnailholder = thumbnail
    ? getImageSource(thumbnail)
    : getImageSource(thumb);
      ...
      {placeholderSource ? (
        ...
        <View
          style={[
            ...
            getWidthHeight(restProps.style),
          ]}
        />
      ) : null}
