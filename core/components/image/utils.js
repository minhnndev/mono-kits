...
export const getTintColor = (style, tintColor) =>
  get(mergeStyle(style), 'tintColor', tintColor);

export const getResizeMode = (style, resizeMode) =>
  get(mergeStyle(style), 'resizeMode', resizeMode);
...
export const getWidthHeight = style => ({
  ...mergeStyle(style),
  width: get(mergeStyle(style), 'width', 30),
  height: get(mergeStyle(style), 'height', 30),
});
...
export const checkLocalImage = source =>
  typeof source === 'number' ||
  (typeof source?.uri === 'string' && !source?.uri?.startsWith('http'));
...
export const getUrl = (source, timeCache) => {
  ...
  if (isLocalImage) {
    ...
    url = {
      ...source,
      uri: getUrlByTime(source?.uri, source?.timeCache || timeCache),
    };
  } else if (typeof source === 'string' && source?.startsWith('http')) {
    url = {uri: getUrlByTime(source, timeCache)};
  } else {
...
export const generatePreLoad = source =>
  source.map(item => ({...item, uri: getUrlByTime(item.uri, 0)}));
