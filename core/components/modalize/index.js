...
const ModalizeBase = (
     ...
     */
        ...
        const obj = StyleSheet.flatten(tag?.props?.style);
        ...
        return (
            ...
            >
                <Animated.View
                    style={{ zIndex }}
                    onLayout={(e) => handleComponentLayout(e, name, absolute)}
                >
                    {tag}
                </Animated.View>
            </PanGestureHandler>
    ...
    const renderContent = () => {
        ...
        const bounces = passedOnProps?.bounces !== undefined && !passedOnProps?.bounces
            ? passedOnProps?.bounces
            : enableBounces;
        const scrollEnabled = passedOnProps?.scrollEnabled !== undefined && !passedOnProps?.scrollEnabled
            ? passedOnProps?.scrollEnabled
            : keyboardToggle || !disableScroll;
        const scrollEventThrottle = passedOnProps?.scrollEventThrottle || 16;
        const onScrollBeginDrag = passedOnProps?.onScrollBeginDrag;
    ...
    //     } else if (!visible && !isFirstLoad.current) {
