...
export default class NavigationButton extends React.Component {
  renderItem = ({
    ...
    if (title) {
      return (
        <AnimatedText
          style={[
            {fontSize: 16, color: color || tintColor || '#fff'},
            titleStyle,
          ]}>
          {title}
        </AnimatedText>
    ...
    if (icon) {
      return isImage(icon) ? (
        <AnimatedImage
          source={
            ValueUtil.getImageSource(icon) ||
            ValueUtil.getImageSource(icons.ic_back.uri)
          }
          style={[
            {
              width: 24,
  ...
  renderBadge({badge, badgeStyle, titleBadgeStyle}) {
    ...
    return (
      <View pointerEvents="none" style={[styles.viewBadge, badgeStyle]}>
        <Text.Caption
          numberOfLines={1}
          style={[{color: Colors.white}, titleBadgeStyle]}>
  ...
  renderContent(item) {
    return (
      <View
        key={item?.index ? item?.index?.toString() : 0}
        style={item.containerStyle}>
        <TouchableOpacity
          accessibilityLabel={
            item.accessibilityLabel || this.props.accessibilityLabel
          }
          activeOpacity={0.85}
