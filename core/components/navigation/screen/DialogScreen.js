...
export default class DialogScreen extends React.Component {
  ...
  onTouchOutside = () => {
    const closeOnTouchOutside = get(
      this.props,
      'route.params.params.closeOnTouchOutside',
      true,
    );
    const onClosePopup = get(
      this.props,
      'route.params.params.onClosePopup',
      () => {},
    );
  ...
  render() {
    ...
    const {
      params: {dragDisabled = true, gestureModal = false, gestureModalProps},
      params,
    } = routeParams;
    const {visible} = this.state;
    const isBottomModal = routeParams.position === 'bottom';
    const ModalComp = isBottomModal
      ? gestureModal
        ? GestureModal
        : BottomModal
      : Modal;
