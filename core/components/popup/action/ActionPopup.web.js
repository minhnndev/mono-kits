...
export default class ActionPopup extends React.Component {
      ...
      return (
        <View key={button.key} style={button.containerStyle}>
          <TouchableOpacity
            ...
            {subTitle ? (
              <Text style={styles.subTextDefaultTextStyle}>{subTitle}</Text>
            ) : (
              <View />
            )}
          </TouchableOpacity>
  ...
  renderCloseButton = () => {
    ...
    if (closeButton.component) {
      return <View style={styles.buttonEmpty}>{closeButton.component}</View>;
    }
  ...
  renderArrayButton = () => {
    ...
    if (buttons) {
      ...
      for (let i = buttons.length - 1; i >= 1; i--) {
        ...
        if (button.component) {
          views.push(
            <View style={styles.buttonEmpty}>{button.component}</View>,
          );
        } else {
          const convertButton = {
            key: i,
  ...
  renderTitle = () => {
    const {title, titleStyle, titleViewStyle, titleUnderLineStyle} = this.props;
    return title ? (
