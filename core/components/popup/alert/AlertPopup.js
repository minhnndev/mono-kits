...
export default class AlertPopup extends React.Component {
  ...
  renderImageHeader() {
    const {header, headerWrapperStyle, headerResizeMode, headerStyle} =
      this.props;
    const {dimensions} = this.state;

    const popupWidth = dimensions?.width
      ? dimensions?.width * 0.87
      : POPUP_WIDTH;
    if (header) {
      return (
      ...
      if (typeof title === 'string') {
        return (
          <View style={[styles.headerFrame, {paddingTop: header ? 20 : 30}]}>
            <Text.H4
              weight="bold"
              style={[styles.defaultTitleStyle, contentStyle.titleStyle]}>
              {title}
            </Text.H4>
          </View>
        );
  ...
  renderBody() {
    const {body, title, header, contentStyle, renderBody} = this.props;
    if (renderBody && typeof renderBody === 'function') {
      return (
        <View
          style={[styles.bodyFrame, {paddingTop: title || header ? 20 : 30}]}>
          {renderBody()}
        </View>
      );
    }
        ...
        return (
          <View
            ...
            <Text.H4
              weight="regular"
              style={[
                styles.defaultBodyStyle,
                contentStyle && contentStyle.bodyStyle,
              ]}>
              {body}
            </Text.H4>
          </View>
