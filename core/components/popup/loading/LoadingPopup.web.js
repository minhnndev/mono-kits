...
export default class LoadingPopup extends React.Component {
  ...
  renderLoading() {
    const {icon, color, size, iconStyle} = this.props;
    const iconViewStyle = {
      ...
      <View style={iconViewStyle}>
        ...
        {icon ? (
          <View style={[styles.iconVIew, iconViewStyle]}>
            <Image style={[innerIconStyle, iconStyle]} source={icon} />
          </View>
        ) : null}
      </View>
    );
    ...
    return (
      <View style={styles.container}>
        {doneLoading === true && showDone ? (
          <Image resizeMode="contain" source={Icons.ic_success_process_white} />
        ) : (
          this.renderLoading()
        )}
        {this.renderText()}
