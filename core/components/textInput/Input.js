...
export default class Input extends React.PureComponent {
        ...
        i < value.length ? (
          ...
          <View
            ...
            style={[
              styles.dot,
              i < value.length
                ? [styles.filledDot, {backgroundColor: 'transparent'}]
                : styles.emptyDot,
            ]}
          />
        ),
  ...
  renderDots = () => {
    ...
    for (let i = 0; i < maxLength; i += 1) {
      textUnderlines.push(
        i < value.length ? (
          <View key={`k_${i}`} style={[styles.dot, styles.filledDot]} />
        ) : (
          <View
            key={`k_${i}`}
            style={[
              styles.dot,
              i < value.length ? styles.filledDot : styles.emptyDot,
            ]}
          />
        ),
      );
  ...
  getText = (text = '', isDefault) => {
    ...
    if (
      ...
      valueFormatted =
        inputType === 'percent'
          ? NumberUtils.formatNumberToPercent(positiveNumberInput, currencyUnit)
          : NumberUtils.formatNumberToMoney(positiveNumberInput, currencyUnit);
      ...
      if (
        ...
        const {value: newText} = this.getText(
          valueFormatted.slice(0, maxLength + currencyUnit?.length - 1),
        );
  ...
  onChangeText = (text, isDefault = false) => {
    ...
    if (
      maxLength &&
      text?.length >
        (inputType === 'money' ? maxLength + currencyUnit?.length : maxLength)
    ) {
      return;
    }
    /**
      ...
      () => {
        if (onChangeText && typeof onChangeText === 'function') {
          if (inputType === 'money' || inputType === 'percent') {
            onChangeText(
              NumberUtils.formatMoneyToNumber(value, currencyUnit).toString(),
            );
          } else {
            onChangeText(value);
          }
  ...
  renderInput = (isInput = true) => {
    ...
    if (
      ((useErrorMessageInRegex && !validated) || !useErrorMessageInRegex) &&
      errorMessage
    ) {
          ...
          <View
            ...
            <View
                ...
                <TouchableOpacity
                  ...
                  <Image
                    ...
                    style={[
                      styles.iconDefault,
                      {
                        marginTop: multiline ? Spacing.M : Spacing.XS,
                      },
              ...
              {initWithDots && !secureTextEntry ? (
                <TouchableOpacity
                  style={[
                    styles.passwordView,
                    {
                      justifyContent: convertStyle[textAlign] || 'flex-start',
                    },
                  ]}
                  activeOpacity={1}
                  onPress={this.focus}>
              ...
              {isInput && !disabled ? (
                <TextInput
                  ...
                  style={[
                    ...
                    initWithDots && {
                      color: 'transparent',
                      letterSpacing: !secureTextEntry ? 4.4 : -99,
                    },
              ...
              <View style={hasRightView ? styles.rightIcons : {}}>
                ...
                {/* {currencyUnit ? (
                            <Text style={[styles.currency, mergeStyle(currencyStyle)]}>
                                {currencyUnit}
                            </Text>
                        ) : <View />} */}
                {(cancellable && value && value?.length > 0 && focused) ||
                  ...
                  <TouchableOpacity
                    ...
                    <Image
                      ...
                      style={[
                        styles.iconDefault,
                        {
                          marginTop: multiline ? Spacing.L : Spacing.XS,
                        },
                        styles.iconClose,
                        (icon || rightComponent) && {
                          marginRight: 7,
                ...
                <TouchableOpacity
                  ...
                  {floatingIcon && (
                    <Image
                      ...
                      style={[
                        styles.floatingIcon,
                        ValueUtil.mergeStyle(floatingIconStyle),
                      ]}
                    />
                  )}
                </TouchableOpacity>
            ...
            {showLength && maxLength && (
              <View style={styles.lengthContainer}>
                <Text
                  style={
                    styles.lengthText
                  }>{`${value.length}/${lengthText.maxLength}`}</Text>
              </View>
            )}
          </View>
          {typeof errorMessageDisplay === 'string' && !hideMessageError ? (
            <View style={[styles.errorMessageView, errorMessageContainerStyle]}>
  ...
  render() {
    const {onPress, textStyle, accessibilityLabel} = this.props;
    const textFlex = get(textStyle, 'flex', null);
