...
export default class RoundTextInput extends Component {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    ...
    if (nextProps.value !== undefined && nextProps.value !== prevState.value) {
      if (
        secureTextEntry &&
        prevState.value.length >= maxLength &&
        value.length > prevState.value.length
      )
        return null;
      return {
        value,
  ...
  onChangeText = text => {
    ...
    const {secureTextEntry, maxLength = PASSWORD_MAX_LENGTH_DEFAULT} =
      this.props;
    if (
      secureTextEntry &&
  ...
  onRightIconPress = () => {
    ...
    if (onRightIconPress && typeof onRightIconPress === 'function')
      onRightIconPress();
  ...
  renderTextUnderline = () => {
    ...
    for (let i = 0; i < maxLength; i += 1) {
      if (valueArray[i]) {
        dots.push(
          <View key={`k_${i}`} style={styles.underlineText}>
            <Text style={styles.textUnderline}>{valueArray[i]}</Text>
          </View>,
        );
      } else {
        dots.push(
  ...
  render() {
    ...
    const textColor =
      !secureTextEntry && initWithDots
        ? get(textStyle, 'color', '#222222')
        : 'transparent';
    const showDots = secureTextEntry && (initWithDots || value.length > 0);
    const hasLeftIcon =
      typeof leftIcon === 'number' ||
      (leftIcon && leftIcon?.uri) ||
      typeof leftIcon === 'string';
    const hasRightIcon =
      typeof rightIconSource === 'number' ||
      (rightIconSource && rightIconSource?.uri) ||
      typeof rightIconSource === 'string';
    ...
    return (
      <View style={[styles.container, ValueUtil.mergeStyle(style)]}>
        <View
          style={[
            styles.iconViewLeft,
            {marginRight: reduceMarginRight ? 0 : 8},
          ]}>
          {hasLeftIcon ? (
            <TouchableOpacity
              hitSlop={hitSlop}
        ...
        <View
          style={[
            ...
            {marginLeft: reduceMarginLeft ? 0 : 8},
          ]}>
          {hasRightIcon ? (
            <TouchableOpacity
              hitSlop={hitSlop}
              ...
              <Image
                ...
                style={[
                  styles.icon,
                  {backgroundColor: bgColorContainer},
                  ValueUtil.mergeStyle(rightIconStyle),
                ]}
              />
            </TouchableOpacity>
          ) : (
        ...
        <View style={{}}>{rightComponent || <View />}</View>
      </View>
    );
  }
}
