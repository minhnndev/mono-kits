...
export default class UnderlineTextInput extends Component {
    ...
    if (nextProps.value !== undefined && nextProps.value !== prevState.value) {
      ...
      if (inputType === 'money' && nextProps.value.toString().length > 0) {
        valueFormatted = NumberUtils.formatNumberToMoney(
          NumberUtils.formatMoneyToNumber(nextProps.value),
        );
  ...
  getText = text => {
    ...
    if (inputType === 'money' && text.toString().length > 0) {
      valueFormatted = NumberUtils.formatNumberToMoney(
        NumberUtils.formatMoneyToNumber(text),
      );
    } else if (inputType === 'date') {
  ...
  onChangeText = (value = '') => {
    ...
    this.setState({value: valueFormatted}, () => {
      ...
      if (onChangeText && typeof onChangeText === 'function')
        onChangeText(
          inputType === 'money'
            ? NumberUtils.formatMoneyToNumber(value)
            : value,
        );
      /**
  ...
  render() {
    ...
    const hasIcon =
      rightIconSource &&
      (typeof rightIconSource === 'number' || rightIconSource.uri);
    const showRightIcon = (focused || alwaysShowRightIcon) && value.length > 0;
    const rightIconView = (
      <Image
        source={clearOnRightPress ? Icons.ic_close_24 : rightIconSource}
      ...
      <View style={style}>
        {floatingValue || (maxLength && maxLengthText) ? (
          <Text
            ...
            {(value.length > 0 ? floatingValue : '') ||
              (value.length
                ? `${maxLengthText} (${value.length}/${maxLength})`
                : ' ')}
          </Text>
        ) : (
          <View />
            ...
            <View>
              {typeof onRightIconPress === 'function' || clearOnRightPress ? (
                <TouchableOpacity
                  ...
                  disabled={
                    !(
                      typeof onRightIconPress === 'function' ||
                      clearOnRightPress
                    )
                  }>
                  {rightIconView}
        ...
        />
        {(isError || !regex || forceDisplayError) &&
