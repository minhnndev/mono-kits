...
import { RNGestureHandler } from '@momo-kits/core';
...
class DraggableFlatList extends React.Component {
  ...
  touchInit = new Value(0); // Position of initial touch

  activationDistance = new Value(0); // Distance finger travels from initial touch to when dragging begins

  touchAbsolute = new Value(0); // Finger position on screen, relative to container
  ...
  activeIndex = new Value(-1); // Index of hovering cell

  spacerIndex = new Value(-1); // Index of hovered-over cell

  isHovering = greaterThan(this.activeIndex, -1);

  activeCellSize = new Value(0); // Height or width of acctive cell

  activeCellOffset = new Value(0); // Distance between active cell and edge of container
  ...
  measureCell = (key) => new Promise((resolve) => {
      ...
      // @ts-ignore
          ...
          const reason = !ref
              ...
              : !flatListNode
                  ? 'no flatlist node'
                  : 'invalid ref';
          if (this.props.debug) console.log(`## can't measure ${key} reason: ${reason}`);
  ...
  scrollToAsync = (offset) => new Promise((resolve) => {
      ...
      if (flatlistRef) {
          const flatlistNode = 'scrollToOffset' in flatlistRef
              ? flatlistRef
  ...
  renderOnPlaceholderIndexChange = () => (
      <Animated.Code dependencies={[]}>
          {() => block([
              onChange(
                  this.spacerIndex,
                  call([this.spacerIndex], ([spacerIndex]) => this.props.onPlaceholderIndexChange(spacerIndex)
                  )
              ),
          ])}
      </Animated.Code>
...
class RowItem extends React.PureComponent {
  drag = () => {
      const {
          drag, renderItem, item, keyToIndex, itemKey, debug
      } = this.props;
