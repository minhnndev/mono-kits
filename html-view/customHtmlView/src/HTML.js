...
import {
    cssStringToRNStyle, _getElementClassStyles, cssStringToObject, cssObjectToString, computeTextStyles
} from './HTMLStyles';
import {
    BLOCK_TAGS,
    TEXT_TAGS,
...
import { HtmlParser } from '@momo-kits/core';

const { DomHandler, Parser } = HtmlParser
export default class HTML extends PureComponent {
    ...
    componentDidUpdate(prevProps, prevState) {
        const {
            html, uri, renderers, tagsStyles, classesStyles
        } = prevProps;
        const doParseDOM = false;
    ...
    mapDOMNodesTORNElements(DOMNodes, parentTag = false, props = this.props) {
        const {
            ignoreNodesFunction, ignoredTags, alterNode, alterData, alterChildren, tagsStyles, classesStyles
        } = props;
        const RNElements = DOMNodes.map((node, nodeIndex) => {
            ...
            if (type === 'tag') {
                ...
                return {
                    wrapper, children, attribs, parent, tagName: name, parentTag
                };
            }
        })
            ...
            .map((parsedNode, nodeIndex) => {
                const {
                    wrapper, children, attribs, tagName
                } = parsedNode;
            ...
            .map((parsedNode, nodeIndex) => {
                const {
                    wrapper, attribs, tagName, children
                } = parsedNode;
    ...
    renderRNElements(RNElements, parentWrapper = 'root', parentIndex = 0, props = this.props) {
        ...
        return RNElements && RNElements.length ? RNElements.map((element, index) => {
            const {
                attribs, data, tagName, parentTag, children, nodeIndex, wrapper
            } = element;
            const Wrapper = wrapper === 'Text' ? Text : View;
            const key = `${wrapper}-${parentIndex}-${nodeIndex}-${tagName}-${index}-${parentTag}`;
            const convertedCSSStyles = attribs && attribs.style
                ? cssStringToRNStyle(
                    attribs.style,
            ...
            if (this.renderers[tagName]) {
                const customRenderer = typeof this.renderers[tagName] === 'function'
    ...
    render() {
        const {
            allowFontScaling, customWrapper, remoteLoadingView, remoteErrorView
        } = this.props;
        ...
        if (!RNNodes && !loadingRemoteURL && !errorLoadingRemoteURL) {
            ...
            return remoteErrorView
                ...
                : (
                    <View style={{ flex: 1, alignItems: 'center' }}>
                        <Text allowFontScaling={allowFontScaling} style={{ fontStyle: 'italic', fontSize: 16 }}>
                            Could not load
                            {this.props.uri}
                        </Text>
                    </View>
